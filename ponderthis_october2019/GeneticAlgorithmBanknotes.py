import random
import operator
from src.Models import MinSetGenerator, MinSetChecker
...
class GeneticAlgorithmBanknotes:
    ...
    in the 'money_range') takes a maximum value
    Attributes:
    ...
    def __init__(self, params, money_range):
    ...
    def fitness(self, individual):
        ...
        for i in range(self.money_min, self.money_max + 1):
            ...
            if MinSetChecker(self.number_of_genes, individual, i, q).is_exist_another_solution():
                ...
                result_all.append(s)
        up = self.money_max - self.money_min + 1
        down = 0.0
        for item in result_all:
            down += result_all.count(item)
        probability = up / (down + self.eps) * 100
        return probability
    ...
    def generate_first_population(self):
        ...
        :return: population
        """
        population = [[1, 5, 10, 20, 50]]
        for i in range(1, self.size_population):
    ...
    def compute_population(self, population):
        ...
        values_sorted = sorted(values_computed.items(), key=operator.itemgetter(1), reverse=True)
    ...
    def generate_population(self, generation_last):
        ...
        f.write(str([population_sorted[i] for i in range(0, self.size_population)]) + "\n")
        f.write(str([values_sorted[i] for i in range(0, self.size_population)]) + '\n')
        f.close()
...
if __name__ == '__main__':
    GeneticAlgorithmBanknotes([5, 21, 1, 4, 4, 5, 60, 2, 99], [0, 99]).run()